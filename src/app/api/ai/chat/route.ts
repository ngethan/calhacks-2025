...
export async function POST(req: Request) {
	try {
		...
		console.log("[API] Received request with", body.messages?.length, "messages");
...
The user can then accept or reject your suggested changes.`
		...
		console.log("[API] Calling OpenRouter with", messages.length + 1, "total messages");
		...
		messagesToSend.forEach((msg, idx) => {
			console.log(`  ${idx}: role=${msg.role}, contentType=${typeof msg.content}, isArray=${Array.isArray(msg.content)}, contentLength=${msg.content.length}`);
		});
		...
		try {
			...
			console.error("[API] streamText error details:", JSON.stringify(streamError, null, 2));
			throw streamError;
		}
	} catch (error) {
